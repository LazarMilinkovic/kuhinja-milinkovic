...
export function getSlotDates(weekStart: number, slotIndex: 0 | 1 | 2): { start: Date; end: Date } {
  const mon = new Date(weekStart)
  const start = addDays(mon, slotIndex * 2)
  const end = addDays(mon, slotIndex * 2 + 1)
  return { start, end }
}

export function formatSlotDates(weekStart: number, slotIndex: 0 | 1 | 2): string {
  const { start, end } = getSlotDates(weekStart, slotIndex)
